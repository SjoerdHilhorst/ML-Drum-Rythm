...
def prepare_data_for_forecasting2(data, n_steps, split_value=0.7):
...
def get_dataset_with_multiple_songs(data_path, window_size, split_value=0.7):
    ...
    X_train, X_test, y_train, y_test = prepare_data_for_forecasting2(time_series_data, window_size, split_value=split_value)
...
def get_dataset_with_multiple_songs2(data_path, window_size, split_value=0.7):
    time_series_data = np.load(data_path).transpose()
